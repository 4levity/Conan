 ...
 * 
 * This file includes some code from Paul Taylor / Jaudiotagger Copyright (C)2004,2005 licensed under LGPL 2.0, Thanks!
 */

/* Genre list
 * This is the ID3v1 list with additional values as defined by Winamp, this list is also used in Mp4
 * files, note iTunes doesn't understand genres above MAX_STANDARD_GENRE_ID, Winamp does
 */
public class GenreList extends ArrayList<String> implements SettableFromString {
	/**
	 * 
	 */
	private static final long serialVersionUID = -6002591342141212551L;
	static HashMap<Integer,String> idToValue=new HashMap<Integer,String>(147);
	static HashMap<String,Integer> lowercaseValueToId=new HashMap<String,Integer>(147);
   ...
   /**
	...
	static {
	    ...
	    {
	        lowercaseValueToId.put(entry.getValue().toLowerCase(), entry.getKey());
	    }
	}
	...
	public GenreList(int id3Genre) {
		super();
		String gName=GenreList.idToValue.get(id3Genre);
		if(gName!=null) {
			this.add(gName);
		} else {
			this.add(Integer.toString(id3Genre));
		}
	}
	...
	public boolean setFromString(String value) {
		this.clear();
		String[] values=value.split("\\x00");
		for(String v : values) {
			// handle id3v2.2/2.3 genre lists "(xx)(xx)text"
			if(v.matches("\\(\\d{1,3}\\).*")) {
				int endParen=v.indexOf(")");
				if(!addNumericId(v.substring(1,endParen))) {
					// didn't parse this, just add the string
					this.add(v);
				} else if(endParen+1 <v.length()) {
					// did parse an id3 genre, recursively process chain of genre list
					// (this is probably rare)
					setFromString(v.substring(endParen+1));
				}
			// handle id3v2.4 reference to genre ID
			} else if(v.matches("\\d{1,3}")) {
				if(!addNumericId(v)) {
					this.add(v);
				}
	...
	private boolean addNumericId(String numericId) {
		int id=-1;
		try {
	...
	public String toString() {
		...
		for(int i=0;i<this.size();i++) {
			String g=this.get(i);
			Integer id=GenreList.lowercaseValueToId.get(g.toLowerCase());
			if(id!=null) {
				result+="("+id.toString()+")";
			}
			result+=g;
